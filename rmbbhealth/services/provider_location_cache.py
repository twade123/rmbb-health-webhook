...
class ProviderLocationCache:
    ...
    def _save_to_github_format(self):
        ...
        try:
            ...
            # CRITICAL FIX: Add global case mappings to preserve them in GitHub
    ...
    def add_case_mapping(self, case_id, provider_name, contact_id, location_id, external_id=None, product_info=None):
        ...
        Args:
            ...
            product_info: Dict with approved product information for reorder system {name, product_id, q_code}
        """
        ...
        with self.lock:
            ...
            if product_info:
                ...
                print(f"💊 Saved approved product info for case {case_id}: {product_info.get('name')} (ID: {product_info.get('product_id')})")
            
            self.cache["case_mappings"][str(case_id)] = case_mapping_data
    ...
    def get_approved_product(self, case_id):
        """
        Get approved product information for a case ID (for reorder system)
        
        Args:
            case_id: RMBB Health case ID
            
        Returns:
            dict: Product info with name, product_id, q_code, or None if not found
        """
        with self.lock:
    ...
    def get_case_mapping(self, case_id):
        ...
        """
        with self.lock:
            # First try global case mappings (old format)
            case_mappings = self.cache.get("case_mappings", {})
            mapping = case_mappings.get(str(case_id))
            ...
            for provider_key, provider_data in self.cache.items():
                if isinstance(provider_data, dict) and "case_mappings" in provider_data:
                    provider_case_mappings = provider_data["case_mappings"]
                    if str(case_id) in provider_case_mappings:
                        mapping = provider_case_mappings[str(case_id)]
