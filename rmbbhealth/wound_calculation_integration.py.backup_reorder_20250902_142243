...
class WoundCalculationIntegration:
    ...
    def _extract_wound_info(self, case_data: Dict) -> Optional[Dict]:
        """
        Extract wound size information from RMBB Health case data.
        ...
        """
        wound_size_str = case_data.get('wound_size', '')  # e.g., "3x4 cm"
        total_wound_size_str = case_data.get('total_wound_size', '')  # e.g., "12 cm2"
        wound_type = case_data.get('wound_type', '')
        
        logging.info(f"   📏 Raw wound data:")
        logging.info(f"      Wound Size: '{wound_size_str}'")
        logging.info(f"      Total Wound Size: '{total_wound_size_str}'")
        logging.info(f"      Wound Type: '{wound_type}'")
        
        # Extract numerical wound size from total_wound_size field first
        wound_size_cm2 = self._parse_total_wound_size(total_wound_size_str)
        
        # If total_wound_size is not available, calculate from dimensions
        if wound_size_cm2 is None:
            wound_size_cm2 = self._calculate_from_dimensions(wound_size_str)
        
        if wound_size_cm2 is None:
            logging.warning(f"⚠️ Could not extract numerical wound size from case data")
            return None
        
        wound_info = {
            'wound_size_cm2': wound_size_cm2,
            'wound_size_str': wound_size_str,
            'total_wound_size_str': total_wound_size_str,
            'wound_type': wound_type
        }
        
        logging.info(f"   ✅ Extracted wound size: {wound_size_cm2} cm²")
        return wound_info
