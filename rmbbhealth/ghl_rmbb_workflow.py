...
class GHLRMBBWorkflowHandler:
    ...
    DOCUMENT_PROCESSING_FIELDS = {
        ...
        # Case Identification Field (1 field)
    ...
    def submit_to_rmbb_health(self, external_id, contact_id, patient_form_data, location_id):
        """
        ...
        for i, product in enumerate(selected_products):
            ...
            print(f"   📏 Wound Size: {rmbb_case_data.get('wound_size')}")
            print(f"   🩹 Wound Type: {rmbb_case_data.get('wound_type')}")
...
✅ Patient information has been updated in your GHL contact record.
    ...
    def transform_case_data_for_product(self, form_data, patient_id, product):
        ...
        case_data = {
            ...
            "hcpcs": product["q_code"],  # Q-code for the product (e.g., "Q4239")
    ...
    def get_product_id_from_biologic(self, product_info):
        ...
        if is_production:
            ...
            q_code_to_product_id = {
                ...
                "Q4316": 375,  # amchoplast → AmchoPlast
                "Q4164": 374,  # helicoll → Helicoll
                "Q4344": 371   # membrane_wrap_tri_layer → Simplimax (closest available in dev)
            }
        
        q_code = product_info["primary_product"]["q_code"]
        numeric_product_id = q_code_to_product_id.get(q_code, 98)  # Default to Membrane Wrap (98) if not found
    ...
    def update_ghl_contact(self, contact_id, update_data, location_id=None, provider_name=None, display_name=None, add_delay=False):
        ...
        # Contact exists, proceed with update (V1 API uses non-location-scoped endpoint)
        url = f"{self.ghl_base_url}/contacts/{contact_id}"
    ...
    def run_complete_workflow(self, webhook_payload=None):
        ...
        try:
            ...
            rmbb_result = self.submit_to_rmbb_health(external_id, contact_id, patient_data, location_id)
...
Case: {case_id}"""
    ...
    def clear_all_product_fields(self, contact_id, location_id):
        """
        Clear all product size custom fields for reorder processing by dynamically 
        finding fields with 'Units' in the name. This ensures only new product 
        combinations are set, not mixed with old ones.
        ...
        try:
            ...
            # Step 1: Get all custom fields for the location to find product fields
            ...
            if fields_response.status_code != 200:
                logging.error(f"❌ Failed to get custom fields: {fields_response.status_code} - {fields_response.text}")
                return {
                    "success": False,
                    "error": f"Could not retrieve custom fields: {fields_response.text}",
                    "fields_cleared": 0
                }
            
            fields_data = fields_response.json()
            all_custom_fields = fields_data.get('customFields', [])
            
            # Step 2: Filter for product size fields ONLY (exclude Q-code summary fields)
            product_field_ids = []
            for field in all_custom_fields:
                field_name = field.get('name', '')
                # Must contain 'Units' and match a product, but NOT contain Q-code patterns
                if ('Units' in field_name and 
                    any(product in field_name for product in [
                        'AmnioAmp-MP', 'AmnioMaxx', 'Palingen', 'Simplimax', 'Xcell Amnio Matrix',
                        'Dermabind-FM', 'Derm Maxx', 'Esano ACA', 'Membrane Wrap'
                    ]) and 
                    not ('(' in field_name and 'Q4' in field_name and ')' in field_name)):  # Exclude Q-code summary fields
                    
                    product_field_ids.append(field.get('id'))
                    logging.info(f"   🔍 Found product size field: {field_name} (ID: {field.get('id')})")
            
            if not product_field_ids:
                logging.warning(f"⚠️ No product size fields found with 'Units' in name")
                return {
                    "success": True,
                    "message": "No product fields found to clear",
                    "fields_cleared": 0
                }
            
            logging.info(f"🎯 Found {len(product_field_ids)} product size fields to clear")
            
            # Step 3: Prepare custom fields update to clear all product fields
            custom_fields_list = []
            for field_id in product_field_ids:
                custom_fields_list.append({
                    "id": field_id,
                    "value": ""  # Empty string clears the field
                })
            
            # Step 4: Make GHL API call to clear fields
            contact_url = f"https://rest.gohighlevel.com/v1/contacts/{contact_id}"
            payload = {"customField": custom_fields_list}
            
            logging.info(f"🧹 Clearing {len(custom_fields_list)} product fields...")
            response = requests.put(contact_url, headers=headers, json=payload)
    ...
    def update_contact_case_id(self, contact_id, location_id, case_id):
        """
        Store the RMBB Health case_id in the contact's custom field.
        
        Args:
            contact_id: GHL contact ID
            location_id: GHL location ID
            case_id: RMBB Health case ID to store
            
        Returns:
            dict: Success status and update result
        """
        try:
            logging.info(f"💾 Storing case_id {case_id} in GHL contact {contact_id}")
            ...
            if not api_key:
                return {
                    "success": False,
                    "error": f"No API key found for location {location_id}",
                    "case_id_updated": False
                }
            
            # Update contact with case_id custom field (V1 API uses non-location-scoped endpoint)
            contact_url = f"https://rest.gohighlevel.com/v1/contacts/{contact_id}"
            headers = {
                "Authorization": f"Bearer {api_key}",
                "Content-Type": "application/json",
            ...
            payload = {
                "customField": [
                    {
                        "id": "M9Dap36CFurN03kQALVC",  # RMBB Case ID field
                        "value": str(case_id)
                    }
                ]
            }
            
            logging.info(f"📝 Updating contact with case_id: {case_id}")
            response = requests.put(contact_url, headers=headers, json=payload)
            
            if response.status_code == 200:
                logging.info(f"✅ Successfully stored case_id {case_id} in contact custom field")
                return {
                    "success": True,
                    "message": f"Case ID {case_id} stored in contact",
                    "case_id_updated": True,
                    "case_id": case_id
                }
            else:
                error_msg = f"GHL API error {response.status_code}: {response.text}"
                logging.error(f"❌ Case ID update failed: {error_msg}")
                return {
                    "success": False,
                    "error": error_msg,
                    "case_id_updated": False
            ...
            logging.error(f"❌ Error updating case_id custom field: {str(e)}")
            return {
                "success": False,
                "error": str(e),
                "case_id_updated": False
