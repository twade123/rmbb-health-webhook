...
✅ Patient information has been updated in your GHL contact record.
        ...
        for field_name, product_info in products.items():
            ...
            if cm2_value and cm2_value != "0":
                try:
                    ...
                    if cm2_float > 0:
                        # Use appropriate ID based on environment
                        product_id = product_info.get("dev_id", product_info.get("prod_id"))
    ...
    def get_product_id_from_biologic(self, product_info):
        ...
        q_code = product_info["primary_product"]["q_code"] if "q_code" in product_info["primary_product"] else product_info["primary_product"]["name"].split()[-1]
        numeric_product_id = q_code_to_product_id.get(q_code, 98)  # Default to Membrane Wrap (98) if not found
...
Case: {case_id}"""
    ...
    def update_contact_case_id(self, contact_id, location_id, case_id):
        ...
        try:
            ...
            # Update contact with case_id custom field
            contact_url = f"https://rest.gohighlevel.com/v1/contacts/{contact_id}"
            headers = {
                "Authorization": f"Bearer {api_key}",
                "Content-Type": "application/json",
