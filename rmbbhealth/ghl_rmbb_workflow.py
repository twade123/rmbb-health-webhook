...
✅ Patient information has been updated in your GHL contact record.
    ...
    def process_approval_document_with_extraction(self, case_id, contact_id, location_id, provider_name, status_context=None):
        """
        Wrapper method that finds and processes ALL documents for a case
        Called by webhook_handler.py - discovers ALL documents and extracts content to GHL fields
        Processes approvals, denials, appeals, and any other document added to the case
        ...
        if status_context:
            logging.info(f"📄 Starting STATUS-TRIGGERED document processing for case {case_id}")
            ...
            logging.info(f"📄 Starting general document processing for case {case_id}")
        
        try:
            # Step 1: Get all files for the case using FileService
            from services.file_service import FileService
            file_service = FileService(self.rmbb_client)
            
            files_result = file_service.get_all_files(team_id=self.rmbb_team_id, case_id=case_id)
            
            if not files_result or not isinstance(files_result, list) or len(files_result) == 0:
                return {
                    "success": True,
                    "message": f"No files found for case {case_id}",
                    "files_processed": 0,
                    "document_type": None,
            ...
            logging.info(f"📋 Found {len(files_result)} files for case {case_id}")
            
            # Step 2: Process each file that looks like an approval document
            processed_files = 0
            processing_results = []
            ...
            for file_info in files_result:
                ...
                # Check if this is a processable document (ANY document type)
                if self._is_processable_document(file_name):
                    logging.info(f"📄 Processing document: {file_info.get('name')} (ID: {file_id})")
                    
                    # Get file download URL
                    file_content = file_service.view_file(
                        team_id=self.rmbb_team_id, 
                        case_id=case_id, 
                        file_id=file_id
                    )
                    ...
                    if isinstance(file_content, dict) and 'link' in file_content:
                        ...
                        # Process this specific document
                        ...
                        if processing_result["success"]:
                            processed_files += 1
                            # Capture latest document metadata for return
                            latest_document_type = processing_result.get("document_type")
                            latest_approval_status = processing_result.get("approval_status")
                            
                            processing_results.append({
                                "file_name": file_info.get('name'),
                                "file_id": file_id,
                                "status": "success",
                                "document_type": latest_document_type,
            ...
            if processed_files > 0:
                # Step 3: Apply status-specific workflow tags after successful document processing
                workflow_tags_applied = []
                if status_context and status_context.get('workflow_tags'):
                    # Get API key for tag operations
                    from services.provider_location_cache import get_provider_cache
                    provider_cache = get_provider_cache()
                    ...
                    if sub_account_api_key:
                        ...
                        logging.warning(f"⚠️ No API key found for applying workflow tags")
                ...
                return {
                    "success": True,
                    "message": "No processable documents found (PDF, HTML, DOC/DOCX)",
                    "files_processed": 0,
                    "processing_results": processing_results,
                    "document_type": None,
                    "approval_status": None,
    ...
    def _map_document_data_to_ghl_fields(self, extracted_data, case_id, document_name):
        ...
        field_mappings = {
            ...
            # NEW HYBRID DOCUMENT FIELDS (ADDITIONAL - not replacing existing)
            # VISUAL UNDERSTANDING FIELDS (5 fields - for provider viewing)
            'rmbb_current_patient_info': 'rmbb_current_patient_info_id',
            'rmbb_current_insurance_info': 'rmbb_current_insurance_info_id', 
            'rmbb_current_decision_summary': 'rmbb_current_decision_summary_id',
            'rmbb_current_notes': 'rmbb_current_notes_id',
            'rmbb_current_status': 'rmbb_current_status_id',
            
            # IVR-SPECIFIC EXTRACTION FIELDS (5 fields - for clean automation)
            'rmbb_ivr_patient_data': 'rmbb_ivr_patient_data_id',
            'rmbb_ivr_primary_insurance': 'rmbb_ivr_primary_insurance_id',
            'rmbb_ivr_secondary_insurance': 'rmbb_ivr_secondary_insurance_id',
            'rmbb_ivr_coverage_summary': 'rmbb_ivr_coverage_summary_id',
            'rmbb_ivr_authorization_info': 'rmbb_ivr_authorization_info_id',
            
            # DOCUMENT TRACKING FIELDS (3 fields - for history and context)
            'rmbb_document_history': 'rmbb_document_history_id',
            'rmbb_case_summary': 'rmbb_case_summary_id',
            'rmbb_total_documents': 'rmbb_total_documents_id'
        }
    ...
    def _add_contact_tag(self, contact_id, location_id, api_key, tag_name):
        """Add a workflow trigger tag to a GHL contact"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(f"❌ Failed to add tag: HTTP {response.status_code}")
                logging.error(f"Response: {response.text}")
                return False
